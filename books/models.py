...
class Category(models.Model):
    ...
    is_manga = models.BooleanField(
        default=False
    )  # Manga category လား၊ Book category လား ခွဲရန်
    is_for_shop = models.BooleanField(
        default=False
    )  # Shop မှာပဲ သီးသန့်ပြချင်သော category ဖြစ်ပါက True ပေးရန်
...
class Book(models.Model):
    ...
    is_free = models.BooleanField(
        default=True
    )  # True ဆိုရင် အလကား၊ False ဆိုရင် Paid စာအုပ်
    ...
    audio_file = models.FileField(
        upload_to="protected_audio/", blank=True, null=True
    )  # Audiobook ပါလျှင်
    ...
    drive_file_id = models.CharField(
        max_length=255, blank=True, null=True, help_text="Google Drive File ID (PDF)"
    )
    drive_audio_id = models.CharField(
        max_length=255, blank=True, null=True, help_text="Google Drive File ID (Audio)"
    )
    drive_account_index = models.IntegerField(
    ...
    def save(self, *args, **kwargs):
        ...
        if self.pdf_file and not self.drive_file_id:
            try:
                ...
                file_id = upload_to_drive(
                    self.pdf_file.path, f"{self.title}.pdf", self.drive_account_index
                )
                if file_id:
        ...
        if self.audio_file and not self.drive_audio_id:
            try:
                audio_id = upload_to_drive(
                    self.audio_file.path,
                    f"{self.title}_audio.mp3",
                    self.drive_account_index,
                )
                if audio_id:
...
class PurchasedBook(models.Model):
    ...
    class Meta:
        unique_together = (
            "user",
            "book",
        )  # တစ်ယောက်တည်းက စာအုပ်တစ်အုပ်ကို နှစ်ခါဝယ်စရာမလိုရန်
...
class History(models.Model):
    ...
    book = models.ForeignKey(
        Book, on_delete=models.CASCADE, related_name="view_history"
    )

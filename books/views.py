...
from .models import Book, Payment, UserSubscription, UserProgress
...
try:
    from .models import Category, History, Manga, Chapter, PurchasedBook
    ...
    if request.method == 'POST':
        ...
        if p1 == p2:
            if User.objects.filter(username=uname).exists():
                ...
                messages.success(request, "အကောင့်ဖွင့်ခြင်း အောင်မြင်ပါသည်။ Login ပြန်ဝင်ပေးပါ။")
...
def library_view(request):
    ...
    if request.user.is_authenticated:
        ...
        if sub and sub.has_access():
            is_premium = True
        if PurchasedBook:
            purchased_ids = list(PurchasedBook.objects.filter(user=request.user).values_list('book_id', flat=True))
    ...
    if search_query:
        all_items = all_items.filter(Q(title__icontains=search_query) | Q(author__icontains=search_query))
    ...
    if request.user.is_authenticated and HISTORY_EXISTS and History:
        try:
            user_history = list(History.objects.filter(user=request.user).values_list('book_id', flat=True))
        except:
            user_history = []
...
def read_book(request, book_id):
    ...
    if request.user.is_authenticated and HISTORY_EXISTS and History:
        try:
            History.objects.update_or_create(user=request.user, book=book, defaults={'viewed_at': timezone.now()})
    ...
    if can_access:
        ...
        if not request.user.is_authenticated:
            messages.info(request, "ဤစာအုပ်ကို ဖတ်ရှုရန် ကျေးဇူးပြု၍ Login အရင်ဝင်ပေးပါ။")
            return redirect('login')
        else:
            messages.warning(request, "ဤစာအုပ်ကို ဖတ်ရှုရန် ဝယ်ယူရန် သို့မဟုတ် Premium ဖြစ်ရန် လိုအပ်ပါသည်။")
...
def read_chapter(request, chapter_id):
    ...
    if request.user.is_authenticated:
        sub = UserSubscription.objects.filter(user=request.user).first()
        if sub and sub.has_access(): is_premium_user = True
...
def manga_chapters_api(request, manga_id):
    ...
    chapters = list(manga.chapters.all().values('id', 'chapter_number', 'title', 'is_premium'))
...
def send_telegram_notification(user, note, photo_path):
    ...
    except Exception as e: print(f"Telegram Photo Error: {e}")
...
def listen_book(request, book_id):
    ...
    can_listen = False
    sub = UserSubscription.objects.filter(user=request.user).first()
    if (sub and sub.has_access()) or (PurchasedBook and PurchasedBook.objects.filter(user=request.user, book=book).exists()):
        can_listen = True
...
def update_pdf_page(request):
    if request.method == 'POST':
        try:
            ...
            UserProgress.objects.update_or_create(user=request.user, book_id=data.get('book_id'), defaults={'last_pdf_page': data.get('page', 1)})
...
def update_audio_position(request):
    if request.method == 'POST':
        try:
            data = json.loads(request.body)
            UserProgress.objects.update_or_create(user=request.user, book_id=data.get('book_id'), defaults={'last_audio_position': data.get('position', 0)})
            return JsonResponse({'status': 'ok'})
        except: return JsonResponse({'error': 'Invalid data'}, status=400)
    return JsonResponse({'error': 'Invalid method'}, status=405)
